---
name: rm-guide-instruction-standards
description: >
  Load when making changes to any instruction file — AGENTS.md (local or
  global), SKILL.md files, agent definitions, or CONTEXT.md files that
  contain LLM behavioral rules. Governs tone and register conventions,
  the negative-constraint principle (Zhang et al. 2026), instruction
  file architecture (what goes where), progressive disclosure
  principles, instruction budget awareness, and the maintenance workflow
  for restructuring instruction files. USE FOR: editing AGENTS.md, editing
  any rm-* SKILL.md, writing new skills, restructuring agent instructions,
  moving content between local/global/skills, reducing instruction noise,
  consolidating overlapping rules, fixing register violations, optimizing
  instruction file structure, applying tone conventions.
---
