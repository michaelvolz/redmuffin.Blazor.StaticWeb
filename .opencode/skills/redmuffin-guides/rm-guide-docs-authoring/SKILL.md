...
description: >
  ...
  standalone guides, and any manually created docs. DO NOT USE FOR:
  docs managed by other skills (read-only — see §0), sidenotes
  (use rm-sidenotes), commit messages (use rm-commit), or transient
  scratch notes. Only for docs you intend to keep and search across
  sessions.
---
...
| Command                                         | Purpose                             |
| ----------------------------------------------- | ----------------------------------- |
| `skill({ name: "rm-guide-docs-authoring" })`       | Load authoring rules before writing |
| `skill({ name: "rm-guide-docs-authoring" }) spec`  | Load + create a new spec            |
| `skill({ name: "rm-guide-docs-authoring" }) guide` | Load + create a new guide           |
