---
name: rm-code-quality
description: C# .NET 9 Blazor WASM coding standards — superfluous code removal, method simplicity, async patterns, collection abstractions. Use when writing new code, refactoring, or cleaning up.
version: 1.0
guide-authors:
...
See `rm-async` for the definitive ConfigureAwait rules table
...
See `rm-csharp-functional` for immutable collection conventions
and `rm-di` for DI lifetime rules for collection registrations.

## 7. Logging

See `rm-logging` for structured logging patterns and
...
- `rm-testing` — comprehensive test patterns, test doubles, and file structure
- `rm-naming` — naming conventions for types, members, and test doubles
