...
After loading this skill, read §Quality Gates Workflow (inlined from
rm-quality-gates) and §Review Heuristics (inlined from
rm-review-heuristics). The remaining skills must still be loaded
individually:

```
skill({ name: "rm-code-quality" })
skill({ name: "rm-csharp" })
skill({ name: "rm-architecture" })
skill({ name: "rm-tdd" })
skill({ name: "rm-testing" })
```

rm-code-quality defines code quality rules. rm-csharp-functional is
the functional C# pattern catalog. rm-architecture loads Ousterhout
and Uncle Bob Martin architecture principles. rm-tdd governs test
discipline. rm-testing provides test patterns.
...
- Write all unit tests for extracted methods (rm-testing pattern).
...
exceptions per rm-code-quality §3.
...
Load `rm-code-quality` alongside this skill. It defines the universal
...
### Functional C# Pattern Catalog (rm-csharp-functional)
...
(2026-05-17). See `rm-csharp-functional` for full pattern documentation.
...
pattern-matching dispatchers), see `rm-csharp-functional`. Every
...
1. **Is the code better?** — Did you extract a method just to make it
   ...
   `rm-code-quality` §2.1 (Q1: ≥5 lines, Q2: reads clearly inline)?
...
- `rm-testing` — test patterns for CRAP fixes (pure functions, fakes, scopes)
- `rm-code-quality` — universal code quality principles
- `rm-csharp-functional` — functional C# patterns (LINQ pipelines, FrozenDictionary,
