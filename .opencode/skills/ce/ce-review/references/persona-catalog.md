...
23 reviewer personas organized into always-on, cross-cutting conditional, and stack-specific conditional layers, plus CE-specific agents. The orchestrator uses this catalog to select which reviewers to spawn for each review.
...
| `rm-dotnet-csharp`     | `rm-dotnet-csharp-reviewer`     | C# code, domain logic, public APIs, perf-sensitive code, security-sensitive behavior, or maintainability-critical refactors                      |
| `rm-powershell`        | `rm-powershell-reviewer`        | PowerShell scripts, modules, CI automation, package tooling, or DevOps helpers                                                                   |
| `rm-blazor`            | `rm-blazor-reviewer`            | Blazor components, render modes, component state, lifecycle behavior, authz in Razor components, or rendering performance                        |
| `rm-html-css-blazor`   | `rm-html-css-blazor-reviewer`   | Semantic HTML, CSS, responsive layout, styling, or Blazor markup                                                                                 |
| `rm-uncle-bob-csharp`  | `rm-uncle-bob-csharp-reviewer`  | C# code, architecture boundaries, dependency direction, testability, craftsmanship, or Clean Code / Clean Architecture concerns                  |
...
3. **For each stack-specific conditional persona**, use file types and changed patterns as a starting point, then decide whether the diff actually introduces meaningful work for that reviewer. Do not spawn language-specific reviewers just because one config or generated file happens to match the extension. For the `rm-*` reviewers, keep `rm-blazor-reviewer` distinct from `rm-html-css-blazor-reviewer` when the diff mixes component behavior with markup.
