...
- `docs/brainstorms/*` -- requirements documents created by skill({ name: "ce-brainstorm" })
- `docs/plans/*.md` -- plan files created by skill({ name: "ce-plan" }) (decision artifacts; execution progress is derived from git, not stored in plan bodies)
