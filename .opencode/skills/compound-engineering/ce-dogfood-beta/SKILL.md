...
- `agent-browser` installed. Check:
  ...
  If not installed, run the skill({ name: "ce-setup" }) skill to install dependencies, then resume. Do not continue without it.

## Reusing Compound-Engineering Skills

skill({ name: "ce-dogfood-beta" }) is an orchestrator. Prefer delegating to existing CE skills over re-deriving their behavior:

| When | Skill | Why |
|------|-------|-----|
| Phase 0 isolation | skill({ name: "ce-worktree" }) | Run the dogfood in an isolated worktree so the main checkout stays clean. |
| agent-browser missing | skill({ name: "ce-setup" }) | Installs `agent-browser` and other deps. |
| A failure's root cause is non-obvious | skill({ name: "ce-debug" }) | Systematic root-cause analysis instead of guess-and-check. |
| Committing each fix | skill({ name: "ce-commit" }) | Consistent, well-scoped commit messages. |
| A bug reveals a reusable lesson | skill({ name: "ce-compound" }) | Capture the learning so the team compounds knowledge. |

Reuse skill({ name: "ce-test-browser" })'s mechanics for port detection and dev-server startup (see Phase 3) rather than reinventing them.
...
3. **Offer isolation.** Ask whether to run in a git worktree so the main checkout stays untouched (use the platform's blocking question tool). If yes, hand off to skill({ name: "ce-worktree" }); if no, continue in place.
...
Determine the port (priority: explicit `--port` > `AGENTS.md`/`CLAUDE.md` > `package.json` dev script > `.env*` `PORT=` > default `3000`). If a server is already listening, reuse it; otherwise start the project's dev command in the background and wait for the port to come up. This is the same mechanism skill({ name: "ce-test-browser" }) uses — follow its Phase 5–6 logic.
...
1. Investigate the root cause. If it's non-obvious, use skill({ name: "ce-debug" }).
...
4. Commit the fix with a clear message (use skill({ name: "ce-commit" })). One logical fix per commit.
5. Re-run the failing scenario in the browser to confirm it now passes; then continue the matrix.
6. If the bug carried a reusable lesson, capture it with skill({ name: "ce-compound" }).
...
8. **Learnings** — reusable lessons worth carrying forward (feed substantial ones to skill({ name: "ce-compound" })).
