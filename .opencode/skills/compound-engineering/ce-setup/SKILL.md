...
If the line above resolved to `CLAUDE_CODE`, this is a Claude Code session and `skill({ name: "ce-update" })` is available. Otherwise, omit any `skill({ name: "ce-update" })` references from output.
