...
- **"Yes, but it's my prod credential"** or any indication the connection has write access -> refuse: "For safety the pulse will not query a database with write access, even read-only in intent." Then fire the `question` tool with options: label "Set up read-only access", description "create a read-only replica or user"; label "Skip DB entirely", description "analytics usually covers the pulse". Do not proceed until the user picks the read-only option with verified read-only scope, or the skip option. Do not capture a read-write connection under any framing.
