...
2. **Learnings search** _(elsewhere-software only; skipped by default in elsewhere-non-software)_ — dispatch @compound-engineering/ce-learnings-researcher with the topic summary in case relevant institutional knowledge exists (skill-design patterns, prior solutions in similar shape). Skip for elsewhere-non-software: the CWD's `docs/solutions/` is unlikely to be topically relevant for non-digital topics, and running it risks polluting generation with unrelated engineering patterns.
