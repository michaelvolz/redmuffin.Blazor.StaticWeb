...
Ask what should happen next using the `question` tool (OpenCode). Fall back to numbered options in chat only if the question tool errors. Never silently skip the question.
...
**Fallback menu after persistent failure.** Use the `question` tool (OpenCode). Present these options (omit option (a) if no repo exists at CWD):
