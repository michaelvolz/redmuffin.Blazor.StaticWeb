...
| Brainstorm handoff | skill({ name: "ce-brainstorm" }) | skill({ name: "ce-brainstorm" }) (universal-brainstorming) |
...
Hand off the ideation content to the skill({ name: "ce-proof" }) skill in HITL review mode. This uploads the doc, runs an iterative review loop (user annotates in Proof, agent ingests feedback and applies tracked edits), and (in repo mode) syncs the reviewed markdown back to `docs/ideation/`.
...
- **§6.3 Brainstorm a selected idea.** On a successful Proof return (`proceeded` or `done_for_now`), do **not** stop at the Phase 6 menu — after applying the per-status handling below (including any stale-local pull offer), continue into §6.3's remaining bullets (mark the chosen idea as `Explored`, then load skill({ name: "ce-brainstorm" })). Only the `aborted` branch returns to the Phase 6 menu, since no durable record was written.
...
3. **Brainstorm a selected idea** — load skill({ name: "ce-brainstorm" }) with the chosen idea as the seed. The orchestrator first writes a durable record using the mode default in Phase 5.
...
- Load the skill({ name: "ce-brainstorm" }) skill with the chosen idea as the seed

**Repo mode only:** do **not** skip brainstorming and go straight to skill({ name: "ce-plan" }) from ideation output — skill({ name: "ce-plan" }) wants brainstorm-grounded requirements. In elsewhere modes, ideation (or ideation + Proof iteration) is a legitimate terminal state; brainstorming is optional deeper development of one idea, not a required next rung on an implementation ladder that does not exist in these modes.
...
The skill({ name: "ce-proof" }) skill performs single-retry-once internally on transient failures (`STALE_BASE`, `BASE_TOKEN_REQUIRED`) before surfacing failure. The proof skill's return contract does not expose typed error classes to callers — the orchestrator cannot distinguish retryable vs terminal failures from outside.
...
- acting on an idea routes to skill({ name: "ce-brainstorm" }), not directly to implementation
