---
name: ce-plan
description: "Create structured plans for multi-step tasks -- software features, research workflows, events, study plans, or any goal that benefits from breakdown. Also deepens existing plans with interactive sub-agent review. Use when the user says 'plan this', 'create a plan', 'how should we build', 'break this down', or when a brainstorm doc is ready for planning. Use 'deepen the plan' or 'deepening pass' for the deepening flow. For exploratory requests, prefer skill({ name: "ce-brainstorm" }) first."
...
If multiple source documents match, ask which one to use using the `question` tool (OpenCode). Otherwise, present numbered options in chat and wait for the user's reply before proceeding.
...
- **Bug-shaped prompt** (user describes broken behavior — "fix the bug where X", error message, regression, "doesn't work"). Surface `skill({ name: "ce-debug" })` as a route-out option alongside continuing with skill({ name: "ce-plan" }) whenever the bug surface is reachable (in cwd OR named repo found at another local path). Stay in skill({ name: "ce-plan" }) silently when the named code can't be found anywhere local — paper-planning is the only useful output for unreachable surfaces.
  ...
  **Headless mode**: skip the skill({ name: "ce-debug" }) suggestion menu entirely; default to continuing with skill({ name: "ce-plan" }) (the user's explicit invocation). There is no synchronous user to resolve a route-out choice, and auto-routing to skill({ name: "ce-debug" }) would change the skill mid-flight without authorization.
...
- Ask the user, using the `question` tool (OpenCode), whether to:
...
Fires **only in solo invocation** — when Phase 0.2 found no upstream brainstorm doc AND Phase 0.4 stayed in skill({ name: "ce-plan" }) (did not route to skill({ name: "ce-debug" }), skill({ name: "ce-work" }), or universal-planning) AND Phase 0.5 cleared (no unresolved blockers) AND not on Phase 0.1 fast paths (resume normal, deepen-intent). Each guard is an explicit conditional. Skip Phase 0.7 entirely when any guard fails — brainstorm-sourced invocations defer to Phase 5.1.5 instead.
...
Confirm and I'll proceed to research, drawing on this scope. (You can also redirect to skill({ name: "ce-brainstorm" }) if this is bigger than you initially thought — I'll stop here and load it for you.)
````
...
- **Tools available + user asked**: Dispatch `@compound-engineering@compound-engineering/ce-slack-researcher` with the planning context summary in parallel with other Phase 1.1 agents. If the origin document has a Slack context section, pass it verbatim so the researcher focuses on gaps. Include findings in consolidation.
...
**Leverage @compound-engineering/ce-repo-research-analyst's technology context:**

The @compound-engineering/ce-repo-research-analyst output includes a structured Technology & Infrastructure summary. Use it to make sharper external research decisions:

- If specific frameworks and versions were detected (e.g., Rails 7.2, Next.js 14, Go 1.22), pass those exact identifiers to @compound-engineering/ce-framework-docs-researcher so it fetches version-specific documentation
...
Ask the user only when the answer materially affects architecture, scope, sequencing, or risk and cannot be responsibly inferred. Use the `question` tool (OpenCode).
...
2. **Run deeper doc review** - Walk through the remaining findings interactively (full skill({ name: "ce-doc-review" }) walkthrough)
...
- **Start `skill({ name: "ce-work" })`** — Invoke the `skill({ name: "ce-work" })` skill via the platform's skill-invocation primitive (`Skill` in Claude Code, `Skill` in Codex, the equivalent on Gemini/Pi), passing the plan path as the skill argument. Do not merely tell the user to type `skill({ name: "ce-work" })` — fire the invocation now so the plan executes in this session.
- **Run deeper doc review** — Re-invoke the `skill({ name: "ce-doc-review" })` skill on the plan path **without** `mode:headless` so the interactive routing question and walkthrough fire. After it returns, re-render this menu with refreshed counts so the user can pick a next-stage action.
- **Create Issue** — Detect the project tracker (`gh` for GitHub, `linear` for Linear) and create the issue from the plan file as described under "Issue Creation" in `references/plan-handoff.md`. After creation, display the issue URL and ask whether to proceed to `skill({ name: "ce-work" })` via the `question` tool (OpenCode).
- **Open in Proof (web app) — review and comment to iterate with the agent** — Load the `skill({ name: "ce-proof" })` skill in HITL-review mode with the plan file as `source file`, the plan title as `doc title`, identity `ai:compound-engineering` / `Compound Engineering`, and recommended next step `skill({ name: "ce-work" })`. Then follow the post-HITL resync logic in `references/plan-handoff.md`, which handles the four `skill({ name: "ce-proof" })` return statuses, re-runs `skill({ name: "ce-doc-review" })` after material edits, and falls back gracefully on upload failure.
