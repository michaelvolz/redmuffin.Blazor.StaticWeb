...
function Add-ChangelogMetadata {
    ...
    $metadata += "---"
    $metadata += ""  # Empty line
    $metadata += "*This changelog was automatically generated from $CommitCount commits.*"
...
function Write-ChangelogToFile {
    ...
    try {
        ...
        # Add metadata
        $finalContent = Add-ChangelogMetadata -Content $changelogContent -CommitCount $CommitCount -FilteredCount $FilteredCount
        ...
        if ($success) {
            ...
            Write-Host "Summary: $totalEntries entries across $($CategorizedCommits.Keys.Count) categories" -ForegroundColor Cyan
            
            foreach ($category in $CategorizedCommits.Keys) {
                $count = $CategorizedCommits[$category].Count
                if ($count -gt 0) {
                    Write-Host "  - $category`: $count entries" -ForegroundColor White
                }
            }
        }
