...
Describe "FileGenerator Tests" {
    ...
    Context "Add-ChangelogMetadata Tests" {
        ...
        It "Should add metadata correctly" {
            ...
            $result | Should -Match "---"
            # Updated to match emoji in output
    ...
    Context "Add-ChangelogMetadata with Emojis Tests" {
        
        BeforeAll {
            # Create test config with emoji
            $testConfig = @{
                emojis = @{
                    autoGenerated = "🤖"
                }
            }
            $script:emojiConfigPath = Join-Path $script:TestDir "test-emoji-metadata.json"
            $testConfig | ConvertTo-Json -Depth 10 | Out-File $script:emojiConfigPath -Encoding UTF8
        }
        
        It "Should add emoji to metadata when config exists" {
            $content = "# Changelog`n`nSome content"
            $result = Add-ChangelogMetadata -Content $content -CommitCount 10 -FilteredCount 0 -ConfigPath $script:emojiConfigPath
            
            $result | Should -Match "\*🤖 This changelog was automatically generated from 10 commits\.\*"
        }
        
        It "Should use default emoji when config missing" {
            $content = "# Changelog`n`nSome content"
            $result = Add-ChangelogMetadata -Content $content -CommitCount 5 -FilteredCount 0 -ConfigPath "non-existent.json"
            
            # Should use the default robot emoji
            $result | Should -Match "\*🤖 This changelog was automatically generated from 5 commits\.\*"
        }
    }
