...
Videos.razor.cs (271 lines) and Articles.razor.cs (356 lines) implement the
same page lifecycle for displaying Raindrop.io content: load from cache,
background refresh with change detection, manual refresh with badge state
machine, error handling, image cache population, and image delegate methods.
The only differences between the two pages were the Raindrop collection ID
