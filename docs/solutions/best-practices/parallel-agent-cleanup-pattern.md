...
The `rm-dev-shutdown` skill uses a swarm/team agent pattern: spawn 4 teammates in a single response message for concurrent execution:
...
- `rm-dev-shutdown` — Full cleanup workflow
- `~/.config/opencode/scripts/cleanup-sessions.ps1` — Session database cleanup (separate concern)
