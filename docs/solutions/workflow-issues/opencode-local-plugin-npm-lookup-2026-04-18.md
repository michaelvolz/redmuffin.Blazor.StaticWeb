...
A custom plugin `block-push.js` exists in the project's `.opencode/plugins/` directory. It was added to the `plugin` array in `opencode.json` as a plain string:
...
The `block-push.js` file remains in `.opencode/plugins/` and is automatically loaded.
