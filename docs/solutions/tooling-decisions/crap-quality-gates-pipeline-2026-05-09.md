...
Created `tools/redmuffin.Tools.sln` with one project (`redmuffin.Tools.QualityGates`) using subcommands: `crap`, `scrap`, `dupes`, `arch`, `all`. Installed via local NuGet feed (`tools/nupkgs/`) that never leaves the repo. All gates share one Roslyn workspace — cyclomatic complexity computed once, reused across CRAP and SCRAP.
...
The full discipline is codified in `rm-uncle-bob-martin-agentic-coding` skill. See `docs/adr/0002-quality-gates-toolchain.md` for architecture rationale.
...
- `.opencode/skills/rm-uncle-bob-martin-agentic-coding/SKILL.md` — Full discipline spec
