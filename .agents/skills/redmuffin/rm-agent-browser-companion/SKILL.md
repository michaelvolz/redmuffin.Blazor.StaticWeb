---
name: rm-agent-browser-companion
description: >-
  Browser automation for this app (agent-browser CLI). DEFAULT target: local
  frontend-only http://localhost:5233 synthetic — not full API unless named.
  Use when: agent-browser, live-site QA, snapshot, screenshot, a11y, console
  errors, network HAR, lazy-load proof, FileNotFoundException in browser,
  /api-health, redmuffin.net, navigate local dev, vitals, headed/headless.
  Load BEFORE the first agent-browser command. Co-loads upstream agent-browser.
  Not for bUnit (rm-test-mechanics). Host start/stop: rm-dev-environment +
  rm-dev-shutdown.
contract: references/skill-contract.md
---

# rm-agent-browser-companion

When **editing this skill**, load `references/skill-contract.md` first.

Project overlay for [agent-browser](https://agent-browser.dev/) on
**redmuffin.Blazor.StaticWeb**. Evaluated 2026-06-20 against
https://redmuffin.net (v0.28.0).

Human-readable guide: `docs/agent-browser-guide-2026-06-20.md` (keep in sync
with this skill; agents still load this skill as the runtime source of truth).

## When to load (MANDATORY — immediate, not delayed)

Load **rm-agent-browser-companion** on the **first** turn that needs any of:

- `agent-browser` / live browser QA / snapshot / screenshot / a11y
- local `localhost:5233` or redmuffin.net navigation
- console / network / HAR / lazy-load / WASM route proof
- browser-reported runtime errors (e.g. `FileNotFoundException` in ErrorBoundary)

Load **before** the first `agent-browser` command. Co-load host rules from
`rm-dev-environment` when starting or choosing a host.

Never load for bUnit (`rm-test-mechanics`).

## Co-load upstream (first command in session)

1. Read `~/.agents/skills/agent-browser/SKILL.md`.
2. Run `agent-browser skills get core` (add `--full` for full command reference).

Never use stale `node_modules` skill copies — the CLI is version-matched truth.

---

## 0 — Critical viewpoint

**agent-browser** is the standard browser automation CLI for this project (all OS).
Rust daemon + CDP against **bundled Chromium** from `agent-browser install` — **not** the
user’s Brave browser. **User browser (all OS): Brave only** — never Google Chrome the product.

**Use for:** agent-driven QA, a11y snapshots, annotated screenshots, live-site
navigation, vitals on deployed or local builds.

**Default target:** local **frontend-only** host (`http://localhost:5233`,
synthetic data) — ~99% of tasks. Production and full-stack API are opt-in when
...
`--offline --quick` — checks CLI, bundled Chromium path, stale daemons (auto-cleaned).
...
Never omit `--session` — wrong daemon or orphan bundled-Chromium windows result.
...
- Quoted `'@e7'` works for `get text` / `get value` when session is healthy.
...
- Headless works on redmuffin.net with Blazor `--fn` wait in isolated session.
...
Operational procedure for any browser task (QA, screenshot, nav check, form).

### 3.0 Default local target (99%) — frontend-only

**This is the default** for agent browser work on this app unless the user or
...
Use only when the task targets the live site (or another named origin). Same
session rules and Blazor wait; different base URL.
...
1. Agent-driven QA — routes, snapshot regressions (**default: local frontend-only**, §3.0)
...
5. API Health route — navigate + `console` / `network requests` (still frontend-only unless real API is named)
...
Never use this skill for: bUnit/TUnit (`rm-test-mechanics`, `rm-test-tdd`), or
component architecture docs (`rm-ui-blazor` / `rm-design-architecture`).
