---
name: rm-dev-toolchain
description: installing or troubleshooting dev tools ΓÇö SCSS, JS, dotnet workloads
---
...
| dart-sass                  | 1.99.0               | Required | SCSS ΓåÆ CSS compilation       |
...
# dart-sass (priority: pacman ΓÇö extra repo)
sudo pacman -S dart-sass

# Node.js (priority: mise ΓÇö Omarchy default for dev runtimes)
...
Start ONCE per session. Background process ΓÇö never interact with it during work.
...
3. Edit SCSS ΓåÆ auto-compiled to CSS ΓåÆ Blazor hot reload ΓåÆ browser updates
...
```bash
# 1. SCSS ΓåÆ minified CSS
sass --style=compressed --no-source-map scss/app.scss:wwwroot/css/app.min.css

# 2. JS ΓåÆ minified JS
npx --yes terser wwwroot/js/page-load-timing.js -o wwwroot/js/page-load-timing.min.js -c
...
- The `sass --watch` process has NO port ΓÇö cannot conflict with `dotnet watch`.

---

## Dev Server (Linux ΓÇö systemd)
...
- `sass --watch scss/app.scss:wwwroot/css/app.css` ΓÇö expanded, readable in DevTools
- `sass --style=compressed --watch scss/app.scss:wwwroot/css/app.min.css` ΓÇö minified, ready to commit
- `dotnet watch --launch-profile https` ΓÇö hot reloads C# + detects CSS changes + auto-refreshes browser

Both sass watchers track imported partials automatically. On every SCSS save, both `app.css` and `app.min.css` update in ~0.45s. `app.min.css` is always fresh ΓÇö commit it directly.
...
**`--working-directory` instead of `cd &&`**: The `cd` command in `bash -c` does not reliably propagate to all background children. systemd's `--working-directory` sets the CWD at the unit level ΓÇö before bash forks ΓÇö guaranteeing all child processes inherit the same directory. This is the canonical systemd way.

**`-p TimeoutStopSec=1`**: `dotnet watch` is stateless ΓÇö no database connections, no pending writes. It ignores SIGTERM entirely. A 1-second timeout means SIGKILL fires after 1s (vs default 90s). Stop completes in ~2s.
...
**`app.min.css` always fresh**: Both sass watchers use file-syntax targets (`scss/app.scss:wwwroot/css/app.css` and `scss/app.scss:wwwroot/css/app.min.css`). File-syntax watchers track imported partials (proven). Directory-syntax watchers (`scss:wwwroot/css`) would output `app.css` only ΓÇö never `app.min.css`.
