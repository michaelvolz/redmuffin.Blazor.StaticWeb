...
[Category("Unit")]
public sealed class LoadArticlesHandlerTests
{
    [Test]
    public async Task Load_returns_cached_items_without_calling_api_when_cache_hit()
    {
        ...
        var handler = new LoadArticlesHandler(api, storage);

        var result = await handler.Handle(new LoadArticlesQuery(), CancellationToken.None).ConfigureAwait(false);
    ...
    public async Task Refresh_fetches_even_when_cache_has_items()
    {
        ...
        var api = new RaindropAPI_Fake { ArticlesResult = Result.Success(fresh) };
        var storage = new RaindropItemsStorage_Fake { GetResult = cached };
        var handler = new RefreshArticlesHandler(api, storage);

        var result = await handler.Handle(new RefreshArticlesCommand(), CancellationToken.None).ConfigureAwait(false);
    ...
    public async Task Load_videos_uses_videos_cache_key()
    {
        ...
        var storage = new RaindropItemsStorage_Fake();
        var handler = new LoadVideosHandler(api, storage);

        var result = await handler.Handle(new LoadVideosQuery(), CancellationToken.None).ConfigureAwait(false);
